...
function ConfigureBudget() {
    ...
    const handleDelete = async (item) => {
        ...
        if (confirmDelete) {
            await axios.delete(`http://18.191.247.199:3001/deleteBudget/${item._id}`);
    ...
    const handleSubmit = async (event) => {
        ...
        await axios.post('http://18.191.247.199:3001/addNewBudget', budgetData);
        setBudgetItem('');
        setBudgetAmount('');
    };
    ...
    useEffect(() => {
        ...
        const fetchBudget = async () => {
            const response = await axios.post('http://18.191.247.199:3001/budget',{userID});
